    ...
    <div className = 'card-body d-flex align-items-center justify-content-between'>
      ...
      <div className="d-flex flex-nowrap flex-column align-items-center">
        ...
        ( user && data.users[user] && data.users[user].events && data.users[user].events.includes(event.event_id)) ? (
          ...
          <button data-testid='join-btn' className='btn btn-success bg-opacity-25' style={{width:"6em"}} onClick={ (ev) => (!user) ? alert("Please log in to join the event") : join(event.player_count, event.event_id, user, data.users[user] && data.users[user].events) } >Join</button>
        ) 
        }
      </div>
    </div>
